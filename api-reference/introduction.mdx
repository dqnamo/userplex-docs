...
The Userplex API allows you to send logs, identify users, and track behavior in your applications. All API requests require authentication using a Bearer token.
...
The API allows up to 1,000 requests per minute per API key. For high-throughput applications, use the batch endpoint (`/logs`) to send up to 100 logs in a single request.
