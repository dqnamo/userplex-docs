...
For client-side route changes, call `pageview()` when the pathname updates:
...
export function SeerstackPageview() {
  const pathname = usePathname();

  useEffect(() => {
    if (window.seerstack && typeof window.seerstack.pageview === "function") {
      window.seerstack.pageview();
    }
  }, [pathname]);
...
Render `<SeerstackPageview />` in your layout or top-level client component.
